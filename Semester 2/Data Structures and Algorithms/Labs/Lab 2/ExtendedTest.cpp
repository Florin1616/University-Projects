...
//create a function that tests the setMainDiagonal function from the Matrix 



void testSetMainDiagonal() {
	cout << "Test setMainDiagonal" << endl;
	Matrix m(10, 10);
	m.setMainDiagonal(3);
	assert(m.element(0, 0) == 3);
	assert(m.element(1, 1) == 3);
	assert(m.element(2, 2) == 3);
	

	Matrix m2(5, 6);
	try {
		m2.setMainDiagonal(2);
		assert(m2.element(0, 0) == 2);
	}
	catch (exception&) {
		assert("Matrix is not square");
	}
}
