...
def common_numbers(first_array, second_array):
    common_numbers = []
...
while True:
    ...
    common_numbers = common_numbers(first_array,second_array)
    client_socket.send(struct.pack('!H', len(common_numbers)))
    for i in range(len(common_numbers)):
        client_socket.send(struct.pack('!H', common_numbers[i]))
